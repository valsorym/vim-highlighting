...
syn region  sqlParens       transparent matchgroup=sqlKeyword start="(" end=")"
syn match   sqlParenEmpty   "()"
syn region  sqlParens       transparent matchgroup=sqlSpecial start="\(\<\w\+\>\)\@<=(" end=")"
...
syn region  sqlTypeRegion   matchgroup=sqlSpecial start="\(create\s\+table\s\+[^(]\+\s\+\)\@<=(" end=")" contains=@sqlALL,sqlTypeMatch
syn region  sqlTypeRegion   matchgroup=sqlSpecial start="\(create\s\+\(or\s\+replace\s\+\)\?procedure\s\+[^(]\+\s*\)\@<=(" end=")" contains=@sqlALL,sqlTypeMatch
